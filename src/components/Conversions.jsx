	...
	if (rateData !== null) {
		...
		return (
			<div>
				...
				<hr></hr>
				<button onClick={refetchRates} className="btn btn-1 btn-3">
					Refresh Rate Data
				</button>
				<button className="btn btn-1" onClick={sortRates}>
					Sort Exchange Rates
				</button>
					...
					<div key={rate.id}>
						<div>
							{rate == rateData.bpi.USD.rate_float ? (
								<p>
									1 USD is {usdRateCon} BTC , and 1 BTC is ${rateData.bpi.USD.rate_float}{' '}
									USD
								</p>
							) : rate == rateData.bpi.EUR.rate_float ? (
								<p>
									1 EUR is {eurRateCon} BTC, and 1 BTC is €{rateData.bpi.EUR.rate_float}{' '}
									EUR
								</p>
							) : (
								<p>
									1 GBP is {gbpRateCon} BTC, and 1 BTC is £{rateData.bpi.GBP.rate_float}{' '}
									GBP
								</p>
							)}
						</div>
				...
				<p>
					<u>Convert USD/EUR/GBP to BTC</u>
				</p>
				...
				<input
					...
					value={value}
				></input>
				<button className="btn btn-2" onClick={convertValue}>
					Submit
				</button>
				<br></br>
				<br></br>
				<p>
					<u>Converted value in BTC:</u> {convValue} BTC
				</p>
			</div>
		);
	}
